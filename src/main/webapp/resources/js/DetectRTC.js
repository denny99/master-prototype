...
(function() {
  ...
  function checkDeviceSupport(callback) {
    ...
    if (!navigator.enumerateDevices && window.MediaStreamTrack &&
      ...
      devices.forEach(function(_device) {
        ...
        if (!device.label) {
          ...
          if (location.protocol !== 'https:') {
            if (document.domain.search &&
                document.domain.search(/localhost|127.0./g) === -1) {
              device.label = 'HTTPs is required to get label of this ' +
                  device.kind + ' device.';
            }
          }
        } else {
  ...
  var isNodeWebkit = !!(window.process &&
      (typeof window.process === 'object') && window.process.versions &&
  ...
  [
    ...
    'RTCIceGatherer'].forEach(function(item) {
  ...
  [
    'AudioContext', 'webkitAudioContext', 'mozAudioContext',
    'msAudioContext'].forEach(function(item) {
  ...
  if (DetectRTC.browser.isChrome && DetectRTC.browser.version >= 46 &&
